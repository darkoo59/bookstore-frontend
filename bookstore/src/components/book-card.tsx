...
export function BookCard({
  ...
  const rateBook = async (value: number | null) => {
    ...
    const responseBook = await fetch(
      API_BASE_URL + "/book/byId/" + bookToPresent.id,
      requestOptionsBook
    );
  ...
  return (
    <Card variant="outlined" className="card">
      ...
      <CardContent>
        ...
        >
          {bookToPresent.title}
          <span className="ms-2 text-muted h3">
            {formatCurrency(bookToPresent.price)}
          </span>
        </Typography>
        <Typography sx={{ mb: 1.5 }} color="text.secondary">
          {bookToPresent.author?.name}
        </Typography>
        <Typography variant="body2">
          <i>Publisher: </i>
          {bookToPresent.publisher}
        </Typography>
        <Typography variant="body2">
          <i>Number of pages: </i>
          {bookToPresent.numberOfPages}
        </Typography>
        <Typography variant="body2">
          <i>Average rating: </i>
        ...
        {isAuthenticated() ? (
          <div>
            <Typography variant="body2">
              <i>Your rating: </i>
              {rating != -1 ? rating : <i>not rated yet</i>}
            </Typography>
            <Rating
              name="size-small"
              defaultValue={rating}
        ...
        {isHovered && hover != -1 ? (
          <span className="ms-2 text-muted h3">{hover}</span>
        ) : (
          <span></span>
        )}
      </CardContent>
            ...
            {quantity === 0 ? (
              ...
              >
                ...
                >
                  <button
                    className="secondary-button"
                    onClick={() => decreaseCartQuantity(bookToPresent.id)}
                  >
                    -
                  </button>
                  <div>
                  ...
                  <button
                    className="secondary-button"
                    onClick={() => increaseCartQuantity(bookToPresent.id)}
                  >
                    +
                  </button>
                </div>
                <button
                  ...
                  onClick={() => removeFromCart(bookToPresent.id)}
                >
                  Remove
                </button>
              </div>
