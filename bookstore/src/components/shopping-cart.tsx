...
export function ShoppingCart({ isOpen }: ShoppingCartProps) {
  ...
  const [discount, setDiscount] = React.useState<DiscountDTO>({finalPrice:0.0,message:undefined});
  ...
  const makeDeliveryPaymentOrder = async () => {
    ...
    try {
    const response = await fetch(API_BASE_URL+'/order/delivery-payment', requestOptions)
    toast.success('Order maked succcessfully!', {position: toast.POSITION.BOTTOM_CENTER});
    }catch(err: unknown) {
...
<AppBar position="static">
  ...
  <Stack gap = {3}>
    ...
    {discount.message != undefined && discount.message[0] != '' ? <div className="d-flex align-items-center ms-auto">
